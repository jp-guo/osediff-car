...
def jpeg_encode(x, qf, rounded=True):
    # Assume x is a batch of size (N x C x H x W)
    # [-1, 1] to [0, 255]
    # x = (x + 1) / 2 * 255
    n_batch, _, h, w = x.shape
    ...
    x_luma = x_luma.reshape(n_batch, (h // 8) * (w // 8), 64).transpose(2, 1)
    x_chroma = x_chroma.reshape(n_batch, (h // 16) * (w // 16), 64 * 2).transpose(2, 1)

    fold = nn.Fold(output_size=(h, w), kernel_size=(8, 8), stride=(8, 8))
    x_luma = fold(x_luma)
    fold = nn.Fold(output_size=(h // 2, w // 2), kernel_size=(8, 8), stride=(8, 8))
