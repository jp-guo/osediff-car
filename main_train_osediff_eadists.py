...
def main(args):
    ...
    dists_loss_fn = pyiqa.create_metric('dists', device=accelerator.device, as_loss=True)
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*m_acc):
                ...
                x_pred, lq_latents, _, _ = model_gen(x_src, batch=batch, args=args)
                x_pred = x_pred * 0.5 + 0.5
                x_tgt = x_tgt * 0.5 + 0.5
                dists_loss = dists_loss_fn(x_pred.float(), x_tgt.float())
                edge_loss = dists_loss_fn(
                    edge_detection_model(x_pred),
                    edge_detection_model(x_tgt.float())
                )
                l2_loss = F.mse_loss(x_pred.float(), x_tgt.float(), reduction="mean")
                loss = l2_loss + dists_loss + edge_loss
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    logs["l2_loss"] = l2_loss.detach().item()
                    logs["dists_loss"] = dists_loss.detach().item()
                    logs["edge_loss"] = edge_loss.detach().item()
                    progress_bar.set_postfix(**logs)
                    if not args.debug:
                        # wandb.log({'epoch': epoch, 'loss_l2': logs["loss_l2"], 'loss_lpips': logs['loss_lpips'],
                        #            'loss_d': logs["loss_d"], 'loss_kl': logs["loss_kl"]})
                        wandb.log({'epoch': epoch, 'l2_loss': logs["l2_loss"], 'dists_loss': logs["dists_loss"], 'edge_loss': logs["edge_loss"]})
